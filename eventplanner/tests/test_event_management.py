...
def test_retrieve_public_event_with_valid_event_id_and_current_user():
    ...
    try:
        ...
        event_response = client.post("/events/register", params=event_data,
                                     headers={"Authorization": f"Bearer {access_token}"})
...
def test_returns_list_of_events_with_valid_query_parameters():
    ...
    try:
        ...
        event2 = client.post("/events/register",
                             params={"title": "Event 2", "location": "test", "description": "Description 2",
                                     "public": False}, headers={"Authorization": f"Bearer {access_token}"}).json()
        event3 = client.post("/events/register",
                             params={"title": "Event 3", "location": "test", "description": "Description 3",
                                     "public": True}, headers={"Authorization": f"Bearer {access_token}"}).json()
...
def test_update_event_status_to_public_when_user_is_organizer_or_admin():
    try:
        ...
        response = client.put(f"/events/{event1['id_event']}/admin", params={"username": "test_user1"},
                              headers={"Authorization": f"Bearer {access_token}"})
        ...
        token_response = client.post("/token",
                                     data={"username": user_data1["username"], "password": user_data1["password"]})
        assert token_response.status_code == 200
        access_token = token_response.json()["access_token"]

        # Call the change_event_status endpoint with public=True and token in authorization header
        response = client.post(f"/events/{event1['id_event']}/status", params={"public": False},
                               headers={"Authorization": f"Bearer {access_token}"})
...
def test_user_join_event_successfully():
    try:
        ...
        event_public = client.post("/events/register",
                                   params={"title": "Event 1", "location": "test", "description": "Description 1",
                                           "public": True}, headers={"Authorization": f"Bearer {access_token}"})
        event_private = client.post("/events/register",
                                    params={"title": "Event 2", "location": "test", "description": "Description 2",
                                            "public": False}, headers={"Authorization": f"Bearer {access_token}"})
        ...
        join_public_event = client.get(f"/events/{event_public.json()['id_event']}/join",
                                       headers={"Authorization": f"Bearer {access_token}"})
        join_private_event = client.get(f"/events/{event_private.json()['id_event']}/join",
                                        headers={"Authorization": f"Bearer {access_token}"})

        assert join_public_event.status_code == 200
        assert join_private_event.status_code == 200

        private_ev = event_table.search(events_query.id == event_private.json()["id_event"])
        public_ev = event_table.search(events_query.id == event_public.json()["id_event"])

        assert len(private_ev) != 0
        assert len(public_ev) != 0

        ids_start_user_active_requests_private = [req.start_user for req in private_ev[0]["requests_to_join"]]
...
def test_user_leaves_event_successfully():
    try:
        ...
        response = client.delete(f"/events/{event_public.json()['id_event']}/leave",
                                 headers={"Authorization": f"Bearer {access_token}"})
